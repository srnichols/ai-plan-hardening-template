...
### Go Style
- **Follow `gofmt`**: All code must pass `gofmt` / `goimports`
...
- **Context**: Pass `context.Context` as first parameter to all I/O functions
...
go build ./...                             # Build all
go test ./...                              # All tests
go test -run TestUnit ./...                # Unit tests
...
- [ ] `context.Context` passed to all I/O functions
- [ ] No SQL string concatenation (use parameterized queries)
- [ ] `go vet` and `golangci-lint` pass cleanly
